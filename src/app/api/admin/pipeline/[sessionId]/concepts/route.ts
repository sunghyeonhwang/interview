...
const COMPOSE_SCHEMA = {
  ...
  properties: {
    image_prompt: {
      type: "string",
      description:
        "영문 이미지 생성 프롬프트. 형태를 기하학적으로 구체 서술 + 스타일 지시(flat vector) + 브랜드명 정확 표기 + 팔레트 HEX 포함. 배경 색 지정 문구는 넣지 말 것(라이트/다크는 별도 부가됨).",
    },
...
// 모든 컴포즈 모드가 공유하는 이미지 프롬프트 작성 원칙 —
// 이미지 모델이 목업·변형 시트·사진풍으로 새는 것을 막고 SVG 변환 가능한 결과를 유도한다
const IMAGE_PROMPT_RULES = `

이미지 프롬프트 작성 원칙 (image_prompt에 반드시 반영):
- 형태를 기하학적·구체적으로 서술하라. 무드 형용사 나열 금지 — "elegant modern logo"가 아니라 "a circle formed by two interlocking C-shaped strokes with rounded terminals"처럼 그릴 수 있게 쓴다.
- 스타일 고정 문구 포함: "flat vector logo, clean geometric construction, solid colors, crisp edges". 사진·3D·질감·그림자·복잡한 그라데이션은 금지 문구로 명시한다.
- 단 하나의 로고만: "single logo lockup, centered" — 여러 변형, 로고 시트, 목업, 프레젠테이션 보드가 나오지 않게 금지 문구를 넣는다.
- 텍스트는 브랜드명만, 정확한 철자를 따옴표로 명시한다 (예: the exact text "GRIFF"). 텍스트 없는 심볼이면 "no text, symbol only"를 명시한다.
- 팔레트 HEX 값을 프롬프트 안에 그대로 명시한다.
- 16px 파비콘에서도 판독 가능한 단순성을 유지한다 — 세부 장식보다 실루엣.`;
...
- 무엇을 어떻게 바꿨는지 rationale에 명시한다.${IMAGE_PROMPT_RULES}`,
...
- 개선 지점을 이미지에서 구체적으로 관찰해 명시하고, 그 관찰을 프롬프트에 반영한다 ("더 세련되게" 금지 — "스템 굵기를 균일하게, 좌우 여백 대칭으로"처럼).${IMAGE_PROMPT_RULES}`
        : `너는 브랜드 아이덴티티 아트 디렉터다. 브리프와 선택된 벤치마크 레퍼런스를 바탕으로,
선택 레퍼런스의 좋은 점을 이 브랜드에 맞게 "리디자인"한 로고 컨셉 시안의 이미지 생성 프롬프트를 만든다.
디자인 판단 규칙:
- 레퍼런스를 베끼지 말고 원리(구성·무드·조형 언어)만 가져온다 — 어떤 원리를 가져왔는지 rationale에 명시한다.
- 모티프는 브리프의 키워드·업의 본질에서 도출한다. 업종 클리셰(병원=십자가, 은행=방패)는 브리프가 요구하지 않는 한 피한다.
- "이미 시도한 접근" 목록이 있으면, 그것들과 조형적으로 뚜렷이 다른 새로운 접근(다른 심볼 모티프, 다른 구성, 다른 무드)을 제안한다. 색만 바꾼 변형은 금지.${IMAGE_PROMPT_RULES}`,
      prompt: `## 브리프
...
위 재료로 로고 컨셉 시안의 이미지 프롬프트·제작 의도·팔레트를 JSON으로 작성하라.`,
  ...
  const lightSuffix = `\n\nRender exactly ONE logo, centered on a plain clean white background (light mode). Flat vector style, crisp edges, solid colors. Strictly NO mockups, NO presentation board, NO multiple variations or grid of logos, NO watermark, NO photo or 3D rendering.`;
  const darkSuffix = `\n\nRender exactly ONE logo, centered on a plain very dark charcoal background #0a0c02 (dark mode), with colors adjusted only as needed for legibility on dark. Flat vector style, crisp edges, solid colors. Strictly NO mockups, NO presentation board, NO multiple variations, NO watermark, NO photo or 3D rendering.`;
...
비평 체크리스트 (각각 확인):
① 조형 완성도 — 형태가 정리되어 있고 시각적 균형·광학 보정이 맞는가
② 텍스트 정확성 — 브랜드명 철자·글자 형태가 깨끗한가
③ 브리프 적합성 — 포지셔닝·키워드에 부합하고 "피할 것"을 건드리지 않는가
④ 소형 판독성 — 파비콘 크기로 줄여도 실루엣이 살아남는가
⑤ 형식 — 단일 로고인가 (변형 시트·목업·장식 배경이면 무조건 improve)
verdict 기준: 다섯 항목 모두 준수하면 keep, 하나라도 약하면 improve.
improve일 때 image_prompt는 원본의 좋은 점은 유지하면서 지적 사항만 정밀하게 고친 완전한 프롬프트여야 하며, 형태를 기하학적으로 구체 서술하고 배경 지정 문구는 넣지 않는다.`,
          prompt: `## 브리프
포지셔닝: ${brief.content?.positioning}
키워드: ${(brief.content?.keywords ?? []).join(", ")}
피할 것: ${(brief.content?.anti ?? []).join(", ")}
...
첨부된 1차 결과를 비평하고 JSON으로 답하라.`,
  ...
  try {
    dark = await genImg(
      `Convert this exact logo design to a dark mode version: place it on a plain very dark charcoal background #0a0c02, adjusting colors only as needed for legibility on dark. Keep the logo's shape, composition and proportions IDENTICAL to the input. Exactly ONE logo, centered, flat vector style. NO mockups, NO variations, NO added decoration.`,
      light
    );
  } catch {
